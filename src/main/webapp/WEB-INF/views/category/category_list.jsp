...
<body>
						...
						<div class="x_content">
							<div class="input-group" style="justify-content: space-between;">
								...
								<form:form modelAttribute="searchForm" cssClass="form-inline"
									servletRelativeAction="/category/list" id="search_form">
	...
	<script type="text/javascript">
		...
		function processPage() {
			...
			if(indexPage > 1){
				$('#pagination').append('<li class="page-item"><a class="page-link" href="<c:url value = "/category/list/'+(indexPage-1)+'"/>">Previous</a></li>');
			}else{
			...
			if(totalPage < 6){
				for(var i = 1; i <= totalPage; i++){
					...
					$('#pagination').append('<li class="page-item '+active+'"><a class="page-link" href= "<c:url value = "/category/list/'+i+'"/>">'+i+'</a></li>');
				}
			}else {
    // Show the very first page followed by a "..." at the beginning of the
    // pagination section (after the Previous button)
    			if (indexPage > 2) {
					$('#pagination').append('<li class="page-item"><a class="page-link" href= "<c:url value = "/category/list/1"/>">1</a></li>');
      				if (indexPage > 3) {
						$('#pagination').append('<li class="out-of-range"><a class="page-link" href="<c:url value = "/category/list/'+(indexPage-2)+'"/>">...</a></li>');
      				}
    			}
    ...
    // and pageCutHigh
    			for (let p = pageCutLow; p <= pageCutHigh; p++) {
     				...
     				if (p > totalPage) {
      				...
      				$('#pagination').append('<li class="page-item '+active+'"><a class="page-link" href= "<c:url value = "/category/list/'+p+'"/>">'+ p +'</a></li>');
    			}
    ...
    // section (before the Next button)
    			if (indexPage < totalPage-1) {
      				...
      				$('#pagination').append('<li class="page-item"><a class="page-link" href="<c:url value = "/category/list/'+totalPage+'"/>">'+totalPage+'</a></li>');
    			}
  			}

			if (indexPage < totalPage) {
    			$('#pagination').append('<li class="page-item"><a class="page-link" href= "<c:url value = "/category/list/'+(indexPage+1)+'"/>">Next</a></li>');
  			}else{
